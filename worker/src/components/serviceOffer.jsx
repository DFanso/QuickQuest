...
export default function ServiceOffer({ offer }) {
  ...
  return (
    <div className="md:w-3/5 w-full p-4 bg-white rounded-lg shadow-md my-4" style={{ boxShadow: '0px 0px 4px 0px rgba(0, 0, 0, 0.25)', borderRadius: '10px', background: '#FFF' }}>
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center">
          <div className="icon text-white rounded-full p-2 mr-2">
            <img src={offer.service.category.iconUrl} alt={offer.service.name} className="w-5 h-5" />
          </div>
      ...
      <div className="text-right">
        {isOfferAcceptedOrExpired ? (
          <button className="bg-gray-400 text-white py-2 px-4 rounded-lg cursor-not-allowed" disabled>
            {offer.status}
          </button>
        ) : (
